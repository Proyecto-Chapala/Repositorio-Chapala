"""
================================================================================
SCRIPT DE CARGA INICIAL DE PRODUCTOS QUÃ�MICOS (SEED DATA)
================================================================================
Inyecta en la base de datos (PostgreSQL o SQLite) los 38 productos quÃ­micos
oficiales de la planilla AOS (All Oil Services, C.A.) con sus especificaciones:
- CÃ³digo SKU
- DescripciÃ³n
- Unidad / PresentaciÃ³n
- Libraje
- Gravedad EspecÃ­fica
...
# ConfiguraciÃ³n del entorno de Django
...
def determine_categoria(unidad):
    u = unidad.upper()
    if "GLS" in u or "TAMBOR" in u or "TOTE" in u or "LTS" in u: return "LIQUIDO"
    return "SOLIDO"

def seed_database():
    print("Iniciando inyección de datos para operaciones...")
    creados = 0
    actualizados = 0
    todas_las_listas = PRODUCTOS_AOS + PRODUCTOS_LIQUIDOS + PRODUCTOS_WELLSITE
    
    for item in todas_las_listas:
        libraje_val = parse_decimal(item.get("libraje", "0.0"))
        gravedad_val = parse_decimal(item.get("gravedad_especifica", "1.0"))
        ...
        prod, created = Producto.objects.update_or_create(
            ...
            defaults={
                ...
                "cantidad": parse_decimal(item.get("cantidad", 0)),
                "costo": parse_decimal(item.get("precio_unitario", 0)),
                "categoria": determine_categoria(item["unidad"])
            }
        )
