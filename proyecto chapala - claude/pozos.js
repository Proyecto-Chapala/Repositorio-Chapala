...
(function () {
  ...
  async function cargarPlantillas() {
    const data = await apiFetch('/operaciones/api/pozos/plantillas/');
  ...
  async function guardarPaso1() {
    ...
    const url = state.pozoId
      ? `/operaciones/api/pozos/${state.pozoId}/paso1/`
      : '/operaciones/api/pozos/paso1/';
    ...
    try {
      ...
      history.replaceState(null, '', `/operaciones/pozos/${state.pozoId}/continuar/`);
  ...
  async function guardarPaso2() {
    ...
    try {
      await apiFetch(`/operaciones/api/pozos/${state.pozoId}/paso2/`, {
        method: 'POST',
  ...
  async function guardarPaso3() {
    ...
    try {
      await apiFetch(`/operaciones/api/pozos/${state.pozoId}/paso3/`, {
        method: 'POST',
        body: JSON.stringify(payload),
      });
      goToStep(4);
  ...
  async function renderResumen() {
    const data = await apiFetch(`/operaciones/api/pozos/${state.pozoId}/`);
    const p = data.pozo;
  ...
  async function confirmarPozo() {
    try {
      const data = await apiFetch(`/operaciones/api/pozos/${state.pozoId}/confirmar/`, { method: 'POST' });
      showToast(data.mensaje, 'success');
      window.location.href = `/operaciones/pozos/${state.pozoId}/spud-date/`;
    } catch (err) {
